...
const FALLBACK_KEY_PREFIX = '@secure-draft:';
...
const getLegacyValue = async (key: string) => {
...
export const secureDraftStorage = {
  async getItem(key: string): Promise<string | null> {
    if (await isSecureStoreAvailable()) {
      const secureValue = await SecureStore.getItemAsync(key);
      if (secureValue !== null) return secureValue;

      const legacyValue = await getLegacyValue(key);
      if (legacyValue !== null) {
